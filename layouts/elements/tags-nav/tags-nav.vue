      ...
      <div ref="scrollBody" class="scroll-body" :style="{ left: tagBodyLeft + 'px' }">
        <transition-group name="taglist-moving-animation">
          <Tag
            ...
            :closable="item.path !== $config.homePath"
  ...
  methods: {
    ...
    handleTagsOption(type) {
      if (type.includes('all')) {
        // 关闭所有，除了home
        const res = this.list.filter(item => item.path === this.$config.homePath)
        this.$emit('on-close', res, 'all')
      } else if (type.includes('others')) {
        // 关闭除当前页和home页的其他页
        const res = this.list.filter(item => routeEqual(this.currentRouteObj, item) || item.path === this.$config.homePath)
    ...
    contextMenu(item, e) {
      if (item.path === this.$config.homePath) {
        return
      }
      this.visible = true
