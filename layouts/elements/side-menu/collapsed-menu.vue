<template>
  <Dropdown ref="dropdown" :class="hideTitle ? '' : 'collased-menu-dropdown'" :transfer="hideTitle" :placement="placement" @on-click="handleClick">
    <a class="drop-menu-a" type="text" :style="{ textAlign: !hideTitle ? 'left' : '' }" @mouseover="handleMousemove($event, children)"
      ><common-icon :size="rootIconSize" :color="textColor" :type="parentItem.icon"/><span v-if="!hideTitle" class="menu-title">{{
    ...
    <DropdownMenu ref="dropdown" slot="list">
      <template v-for="child in children">
        <collapsed-menu v-if="showChildren(child)" :key="`drop-${child.name}`" :icon-size="iconSize" :parent-item="child"></collapsed-menu>
        <DropdownItem v-else :key="`drop-${child.path}`" :name="getNameOrHref(child)"
