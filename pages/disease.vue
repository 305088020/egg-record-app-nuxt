  ...
  <div style="background:#eee;padding: 10px">
    ...
    <Modal v-model="modal1" :title="title" :okText="okText" :mask-closable="false" @on-cancel="cancel">
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
        <FormItem label="疾病类型" prop="type">
          <Input v-model="formValidate.type" placeholder="疾病类型"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
...
<script>
import { index, create, update, show, destroy } from '@/api/disease'
export default {
  async asyncData({ $axios }) {
    const cards = await index({ limit: 10, currentPage: 1, sort: 'id,asc' })
  ...
  methods: {
    ...
    edit(id) {
      ...
      show(id).then(response => {
        ...
        this.formValidate.type = response.type
      })
    },
    comfirmDel() {
